...
def process_image(input_image):
    ...
    # Load the pre-trained SwinIR model
    model = SwinIR(upscale=2, in_chans=3, img_size=64, window_size=window_size, img_range=1.,
                   depths=[6, 6, 6, 6, 6, 6], embed_dim=180, num_heads=[6, 6, 6, 6, 6, 6],
                   mlp_ratio=2, upsampler='pixelshuffle', resi_connection='3conv')
    
    pretrained_dict = torch.load("./best_weights.pth", map_location=torch.device('cpu'))
    model.load_state_dict(pretrained_dict)
    model = model.eval()

    # Convert PIL image to numpy array and ensure it's in RGB format
    img_lq = np.array(input_image.convert('RGB')).astype(np.float32) / 255

    # Convert from HWC RGB to CHW RGB
    ...
    # Convert from CHW RGB to HWC RGB and scale to [0, 255]
    output = output.data.squeeze().float().cpu().clamp_(0, 1).numpy()
    output = (output * 255.0).round().astype(np.uint8)
    output_transposed = output.transpose(1, 2, 0)

    # Convert from RGB to BGR for OpenCV compatibility
    output_bgr = cv2.cvtColor(output_transposed, cv2.COLOR_RGB2BGR)
    
    # Convert the numpy array back to PIL image in RGB format (if needed elsewhere in PIL format)
    output_image = Image.fromarray(cv2.cvtColor(output_bgr, cv2.COLOR_BGR2RGB))
